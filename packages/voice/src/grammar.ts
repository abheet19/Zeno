...
function taskTail(command: string): string | null {
  ...
  for (const prefix of TASK_PREFIXES) {
    ...
    if (/^[._/\\-]+[\p{L}\p{N}]/u.test(tail)) continue;
    ...
    if (tail === '' || !/^[\p{L}\p{N}]/u.test(tail)) return tail;
  }
  return null;
}

function taskTitle(tail: string): string {
...
export function parseCommand(command: string): Intent {
  ...
  if (tail !== null) {
    ...
    if (/[\p{L}\p{N}]/u.test(title)) return { kind: 'add_task', title };
