...
export class TranscriptRing {
    ...
    windowMs;
    maxEntries;
    constructor(windowMs = RETENTION_MS, maxEntries = RETENTION_MAX_ENTRIES) {
    ...
    evict(now) {
        ...
        let first = 0;
        while (first < this.lines.length && this.lines[first].at <= cutoff)
            first += 1;
        if (first > 0)
            this.lines = this.lines.slice(first);
        if (this.lines.length > this.maxEntries)
            this.lines = this.lines.slice(this.lines.length - this.maxEntries);
    }
}
/**
...
export class WakeListener {
    ...
    hear(text, final, now) {
        ...
        if (!final || command === '')
            return { kind: 'capturing', partial: command };
        this.current = 'armed';
        this.deadline = 0;
        return { kind: 'command', command, outcome: interpretCommand(command) };
    }
