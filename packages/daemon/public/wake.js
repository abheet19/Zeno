 ...
 */
const WAKE_PAIR_HEAD = new Set(['zee', 'ze']);
const WAKE_PAIR_TAIL = new Set(['no', 'know', 'noh']);
...
export function detectWake(text) {
    ...
    if (afterWake !== undefined && afterWake.norm === WAKE_TAIL)
        j += 1;
    ...
    if (j >= tokens.length && unstripped < tokens.length)
        j = unstripped;
    const head = tokens[j];
    if (head === undefined)
        return { command: '' };
