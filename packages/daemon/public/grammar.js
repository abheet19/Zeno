...
export const APPROVAL_BY_HAND = 'Approval is by hand in the Zeno window — never by voice. Nothing was approved.';
...
export const NO_SAFE_NAME = 'I understood a file proposal but could not derive a safe name from what I heard.';
 ...
 */
export const DELEGATE_NEEDS_A_TASK = 'I heard a build request but not what to build. Say what you want built, in full.';
...
const CONFIRM_PREFIX = /^(?:(?:yes|yeah|yeh|yep|yup|ok|okay|okey|sure|alright|all right|please|just|simply|go ahead and|go on and|can you|could you|would you|will you|you can|you may|i want you to|i need you to|i(?:'|’)?d like you to|i would like you to|let(?:'|’)?s|lets|let us),?\s+)+/i;
...
const OPERATE_CONTROL = /^(?:click|clicks|clicking|press|presses|pressing|hit|hits|tap|taps|push|pushes|select|choose)\s+(?:on\s+)?(?:the\s+)?(?:approve|approval|confirm|confirmation|accept|allow|ok|okay|yes|commit|merge|apply|submit|green(?:\s*light)?|green\s+button)\b/i;
...
const APPROVAL_LEAD = /^(?:approv(?:e|es|ed|ing|al)?|confirm(?:s|ed|ing)?|accept(?:s|ed|ing)?|authori[sz]e?(?:s|d|ing)?|commit(?:s|ted|ting)?|merg(?:e|es|ed|ing)|appl(?:y|ies|ied)|proceed(?:s|ed|ing)?|sign(?:s|ed)?\s+off)\b/i;
 ...
 * still counts.
 */
function isApprovalClause(cmd) {
    const core = cmd.replace(CONFIRM_PREFIX, '');
    const hit = (s) => APPROVAL_LEAD.test(s) || APPROVAL_WHOLE.test(s) || OPERATE_CONTROL.test(s);
 ...
 */
function isApprovalAttempt(cmd) {
    if (isApprovalClause(cmd))
        return true;
    const clauses = cmd
...
const PROPOSE_KIND_FIRST = /^(?:create|add|write|make|new|build|generate|scaffold)\s+(?:a|an|the)?\s*(component|file)\s+(?:called|named)\s+(.+?)(?:\s+(?:that|which|to|for|so that)\s+(.+?))?[.!]?$/i;
/**
 ...
 */
const DELEGATE_PRONOUN_ONLY = /^(?:me\s+)?(?:it|that|this|one|them|these|those|everything|anything|something|the rest|the thing|all of it)$/i;
 ...
 */
export function deriveRelPath(name, kind) {
...
export function parseCommand(command) {
    ...
    const cmd = command
        ...
        .trim();
    if (cmd === '')
        return unrecognized(cmd, EMPTY_COMMAND);
    if (CANCEL.test(cmd))
        return { kind: 'cancel' };
    ...
    if (nameFirst) {
        ...
        if (relPath === null)
            return unrecognized(cmd, NO_SAFE_NAME);
    ...
    if (delegate) {
        ...
        if (job === '' || DELEGATE_PRONOUN_ONLY.test(job))
            return unrecognized(cmd, DELEGATE_NEEDS_A_TASK);
        return { kind: 'delegate', task: cmd.replace(/[.!?]+$/, '').trim() };
    }
    if (GREETING.test(cmd))
        return { kind: 'acknowledge', text: cmd };
