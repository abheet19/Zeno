 ...
 * strictly worse than no screen. So the patterns in `screen-rules.ts` match the
 * SHAPE of an escalation attempt, not merely its vocabulary: "disregard the
 * trailing comma" stays clean while "disregard the previous instructions" does
 * not, and "you are now ready" stays clean while "you are now an unrestricted
 * agent" does not. Every rule is data, in the open, and every finding carries the
 * exact text that tripped it so the owner can judge it themselves rather than take
 ...
 *   fire on the sentence a model would read rather than on its spelling. The fold
 *   (`text-normalize.ts`) keeps a byte map back to the original, so every quote is
 *   still cut from the author's own characters and the owner reads what they
 *   actually wrote.
 *
 ...
 *
 * THE SPLIT. The rule vocabulary — severities, the `Finding`/`ScreenRule` shapes,
 * the phrase matcher and the rule list itself — lives in `screen-rules.ts`, kept
 * separate so it can be read and reviewed as a whole. The folding/quoting engine
 * behind NORMALISE lives in `text-normalize.ts`. This file is what is left: the
 * result shape, the base64 second pass, and `screen()` itself, which drives both
 * of the others.
 */
...
import { fold, quoteOf, type Folded } from './text-normalize.js';

// Re-exported so these stay importable from `./screen.js` exactly as before
// the split. See the file comment above for where each now lives.
export { MAX_QUOTE_CHARS, SCREEN_RULES };
export type { Finding, ScreenRule, Severity, Verdict };
